...
public class test
{
    ...
    public test(ArrayList<Player> p)
    ...
    //create a "dealer" player, set everyones hands, and print the board
    {
        ...
        for (int i = 0; i < 2; i++) //loop to deal two cards to each player/dealer in order
        {
            ...
            for (Player player : p)
            {
                player.getHands().get(0).addToHand(new Card(Suit.CLUBS, Rank.EIGHT));
            }
    ...
    public boolean playerTurn(Hand h, Player p) // need to figure out split betting
    {
        ...
        {
            System.out.println(p.getName() + " your hand " + h.handToString() + " is " + h.scoreToString() + " Would you like to hit(h), double down(d) or stand(s)?");

            if (h.getHand().get(0).rank.value() == h.getHand().get(1).rank.value())
            {
                System.out.println("or would you like to split(p)?");
            }

            choice = input.nextLine();

            if (choice.equalsIgnoreCase("p") && (h.getHand().get(0).rank.value() == h.getHand().get(1).rank.value())) //need to figure out a lot about betting on this condition
            {
